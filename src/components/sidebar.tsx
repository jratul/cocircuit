...
export const SidebarItem = forwardRef(function SidebarItem(
  ...
  const classes = clsx(
    ...
    // Hover
    "data-[hover]:bg-violet-400/5 data-[slot=icon]:*:data-[hover]:fill-violet-200",
    // Active
    "data-[active]:bg-violet-400/5 data-[slot=icon]:*:data-[active]:fill-violet-400",
    // Current
    "data-[slot=icon]:*:data-[current]:fill-violet-400",
    // Dark mode
    ...
    <span className={clsx(className, "relative")}>
      {current && (
        <motion.span
          ...
          className="absolute inset-y-2 -left-4 w-0.5 rounded-full bg-violet-400 dark:bg-white"
        />
      )}
      {"href" in props ? (
...
export function SidebarLabel({ className, ...props }: React.ComponentPropsWithoutRef<"span">) {
  return <span {...props} className={clsx(className, "text-ellipsis")} />;
}
